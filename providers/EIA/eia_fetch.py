...
class EiaFetch(DataPipeline):
    ...
    def load_transformation(self, table_name):
        ...
        try:
            module_name = (
                f"{self.transformations_folder}.{table_name.lower()}_transform"
            )
            ...
            print(
                f"Transformation module not found for table '{table_name}': {e}"
            )
            return None
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            ...
            for future in as_completed(future_to_offset):
                ...
                try:
                    ...
                    df = df[requiredCol].rename(
                        columns={"value": repColNameWith}
                    )
                    ...
                    with db_lock:
                        df.to_sql(
                            table_name, en, if_exists="append", index=False
                        )
                        time.sleep(0.2)
                        print(
                            f"Data loaded successfully for {table_name} with offset {offset}"
                        )
                    ...
                    print(
                        f"An error occurred for {table_name} at Offset {offset}: {response.status_code}"
                    )
    ...
    def validate_column_types(self, df, col):
        ...
        if not pd.api.types.is_numeric_dtype(df[col]):
            raise ValueError(
                "Column 'co2_emission_MMT' should be of numeric type."
            )
